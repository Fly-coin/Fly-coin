...
public:
	...
	bool MultiSend();
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, bool fMultiSendTx, int nSplitBlock, const CCoinControl *coinControl=NULL);
